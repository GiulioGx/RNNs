...
The only situation in which \textit{exploding} gradient can be a problem is  when using learning algorithms like gradient descent with constant step:
...
We can further characterise each path cost noticing that we can separate two components, one that depends only on the weights $w_{qj} \hdots w_{jh}$ and the other that depends both on the weights and the inputs
...
As a final observation on this matter it's worth noticing how a bad initialisation of $\mat{W^{rec}}$ can lead to poor solutions or extremely large convergence time
just because such initialisation imply $\frac{\partial \vec{a}^t}{\partial \vec{a}^k}$ approaching zero norm for $t>>k$. Moreover, even if we somehow provide
an initialisation matrix which is unaffected by this curse, it's certainly possible that we reach such a bad matrix during learning phase.
