...
\begin{remark}
We can see from ReLU's derivative expression that when a neuron is not active it doesn't contribute to the gradient, i.e the derivative with respct to that
neuron is zero. This leads to notice that we cannot learn to 'turn on' a neuron with ReLU activation function. This behaviour is somehow different from what
we would have using sigmoid activation function, for instance, which have always derivative different from zero. Such activation function though
exhibit a good behaviour near zero, they enter \textit{saturation} regions when they are too distant from zero. In such region derivatives are close to zero,
so again we cannot learn anything good from there.
//DISEGNINO DELLE FUNZIONI
\end{remark}
