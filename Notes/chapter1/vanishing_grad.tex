...
\paragraph{Upper bound using singular values}
...
\label{memory_eq}
\end{equation}
Recalling that $\norm{U\cdot \vec{x}}=\norm{\vec{x}}$ for any orthogonal matrix $\mat{U}$ we get
\begin{align}
\norm{\frac{\partial \vec{a}^t}{\partial \vec{a}^k} \cdot \vec{e}_i} &= \norm{ (\prod_{i=t-1}^{k} diag(\sigma'(\vec{a}^i)) \cdot \mat{S}\cdot \mat{D} \cdot \mat{V}^t) \cdot \vec{e}_i}\\
