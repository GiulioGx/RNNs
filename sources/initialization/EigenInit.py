...
class EigenInit(MatrixInit):
    ...
    def __init__(self, seed:int, mean:float=1., std_dev:float=0.01):
        # random generator
        self.__rnd = np.random.RandomState(seed)  # XXX pass the seed in init method
        self.__mean = mean
    ...
    @property
    def infos(self):
        return InfoGroup('eigen init strategy', InfoList(PrintableInfoElement('mean', ':2.2f', self.__mean), PrintableInfoElement('std_dev', ':.2f', self.__std_dev)))
